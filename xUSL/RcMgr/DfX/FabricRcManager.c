 ...
 * @param   BaseAddress        Starting address of the requested MMIO range.
 * @param   Length             Length of the requested MMIO range.
 * @param   Alignment          Alignment bit map.
 * @param   Target             PCI bus number/RootBridge number of the requester.
 * @param   Attributes         Attributes of the requested MMIO range indicating whether
  ...
  )
{
  ...
  RcMgrData = (DFX_RCMGR_INPUT_BLK *)xUslFindStructure(SilContext, SilId_RcManager, 0);
  // assert (RcMgrData);
  if (RcMgrData == NULL) {
  ...
  *BaseAddress = 0;
  ...
  if (Status == SilPass) {
    ...
    // So if there's no enough space, ASSERT
    if (Attributes->MmioType == NON_PCI_DEVICE_BELOW_4G) {
  ...
  for (LogSktLoop = 0; LogSktLoop < SilData->SocketNumber; LogSktLoop++) {
    for (LogRbLoop = 0; LogRbLoop < SilData->RbsPerSocket; LogRbLoop++) {
      ...
      if (MinIoRbCnt-- || (((LogSktLoop + 1) == SilData->SocketNumber) && ((LogRbLoop + 1) == SilData->RbsPerSocket))) {
        ...
        SilSetIoReg4(SilContext, SilData->SocketNumber, RegIndex, PhySktLoop, PhyRbLoop, IoBase, IoSizeForThisRb);
