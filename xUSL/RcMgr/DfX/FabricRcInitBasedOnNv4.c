  ...
  )
{
  ...
  memcpy(ResourceDistribution,
    ...
    sizeof (bool) * PROJ_MAX_SOCKETS_SUPPORTED
    * PROJ_MAX_HOST_BRIDGES_PER_SOCKET
    );
  return SilPass;
}
  ...
  )
{
  ...
  // From RootBridge 0, they're total number of 'ResetTheseRbs' sequential RootBridge, whose MMIO is below Pcie Cfg
 ...
 * @param[in]         PrimaryRootBridge           Primary root bridge index
 * @param[in]         SetDfRegisters              true  - Set DF MMIO registers
 *                                                false - Do Not set DF MMIO registers, just calculate if
 *                                                        user's requirement could be satisfied.
 * @param[in, out]    OverSizeBelowPcieMin        OverSizeBelowPcieMin
  ...
  for (NumberOfRbBelowPcieCfg = 0;
    ...
    while (NextCombination) {
      if (((*EnoughAbovePcieSpaceForPrimaryRb) &&
        ...
        GetAnCombination = SilTryThisCombination(SilData,
          MmioBaseAddrAbovePcieCfg,
          MmioBaseAddrBelowPcieCfg,
          MmioIsAbovePcieCfg,
          false,
  ...
  )
{
  ...
  RCMGR_TRACEPOINT(SIL_TRACE_INFO, "AmdFabric1TbRemap: %d\n", SilData->AmdFabric1TbRemap);
  ...
  if ((SilData->SetRdOnNv) && (SilFabricGetResourceDistribution(SilData, &MmioIsAbovePcieCfg[0]) == SilPass)) {
    // Get distribution information from NV, try it first
    if (SilTryThisCombination(SilData,
      MmioBaseAddrAbovePcieCfg,
      MmioBaseAddrBelowPcieCfg,
      MmioIsAbovePcieCfg,
      SetDfRegisters,
  ...
  if (!LastCombinationWork) {
    if (SilArrangeMmioBelow4G(SilData,
      MmioBaseAddrAbovePcieCfg,
      MmioBaseAddrBelowPcieCfg,
      MmioIsAbovePcieCfg,
      (uint8_t) PrimarySocket,
 ...
 *
 * @details If reserved region for primary RB (PcdAmdBottomMmioReservedForPrimaryRb ~ BottomOfCompat) has not been set,
  ...
  )
{
  ...
  if ((Status != SilPass) || (ApobIp2IpApi == NULL)) {
    ...
    DiePerSkt = DfIp2IpApi->DfGetNumberOfDiesOnSocket(SktNum);
    if (DiePerSkt <= SocMaxDieInfo.MaxSocDiesPerSocketValue) {
      for (uint32_t i = 0; i < TotalSocket; i++) {
        for (uint32_t j = 0; j < DiePerSkt; j++) {
          ...
          DfIp2IpApi->DfAbstractRegAcc((uint8_t)MMIO_ADDRESS_CONTROL0_RE,
            (uint32_t)i,
            MmioPairIndex,
            FABRIC_REG_ACC_BC,
            0,
          ...
          DfIp2IpApi->DfAbstractRegAcc((uint8_t)MMIO_ADDRESS_CONTROL0_WE,
            (uint32_t)i,
            MmioPairIndex,
            FABRIC_REG_ACC_BC,
            0,
          ...
          DfIp2IpApi->DfAbstractRegAcc((uint8_t)MMIO_ADDRESS_CONTROL0_DstFabricID,
            (uint32_t)i,
            MmioPairIndex,
            FABRIC_REG_ACC_BC,
            0,
            DF_ABSTRACT_REG_WRITE,
            (uint32_t) DfIp2IpApi->DfGetHostBridgeSystemFabricID(SktNum, 0, RbNum),
            0
            );
        }
      }
  ...
  )
{
  ...
  for (i = 0; i < TotalSocket; i++) {
    ...
    for (j = 0; j < DiePerSkt; j++) {
       ...
       */
      DfIp2IpApi->DfAbstractRegAcc((uint8_t)X86_IOLIMIT_ADDRESS0_DstFabricID,
        (uint32_t)i,
        RegIndex,
        FABRIC_REG_ACC_BC,
        0,
      ...
      DfIp2IpApi->DfAbstractRegAcc((uint8_t)X86_IOLIMIT_ADDRESS0_IOLimit,
        (uint32_t)i,
        RegIndex,
        FABRIC_REG_ACC_BC,
        0,
  ...
  )
{
  ...
  if (!(*LogToPhyMapInit)) {
    ...
    for (i = 0; i < SocketCount; i++) {
      DieCount = DfIp2IpApi->DfGetNumberOfDiesOnSocket(i);
      RbNumInSkt = 0;
      for (j = 0; j < DieCount; j++) {
        RbCount = DfIp2IpApi->DfGetNumberOfRootBridgesOnDie(i, j);
        for (k = 0; k < RbCount; k++) {
          (pLogToPhyMap + SysRbCount)->PhySktNum = i;
          (pLogToPhyMap + SysRbCount)->PhyRbNum = RbNumInSkt;
          (pLogToPhyMap + SysRbCount)->PciBusBase = DfIp2IpApi->DfGetHostBridgeBusBase(i, j, k);
          (pLogToPhyMap + SysRbCount)->PciBusLimit = DfIp2IpApi->DfGetHostBridgeBusLimit(i, j, k);
