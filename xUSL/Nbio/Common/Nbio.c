  ...
  )
{
  ...
  if (SilGetCommon2RevXferTable(SilId_NbioClass, (void **)(&NbioXfer)) != SilPass) {
    return SilNotFound;
  }
  ...
  DfIp2IpApi->DfGetSystemInfo(&NumberOfProcessors,
    &TotalNumberOfDie,
  ...
  GnbPcieSib = SilCreateInfoBlock(SilId_NbioClass,        // ID
  ...
  if (GnbPcieSib != NULL) {
    ...
    NbioXfer->NbioPcieFixupComplexDefaults(Pcie);
    // Use logical lane numbers to generate physical as well as account for port reversals
    // For use with DXIO
    NbioXfer->NbioPcieFixupPlatformConfig(Pcie);
...
{
  return xUslFindStructure(SilId_NbioClass, NBIOPCIECLASS_INSTANCE);
}
