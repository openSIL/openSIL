...
/**
 * @file  PciStructs.h
 * @brief PCI structures and definitions
 */
...
typedef struct {
  ...
  bool                PIOffsetCalibration;        ///< PI Offset Calibration Enable
} RX_ADAPT_MODE;
...
typedef union {
  ...
  CHANNEL_MAPPING      ChannelMapping;            ///< Channel mapping
...
typedef struct  {
  ...
  uint8_t                HdpIndex;                      /**< Indicates which HDP pin is used
                                                         ...
                                                         *  @li @b 5 - HDP6
                                                         */
  CONN_CHANNEL_MAPPING Mapping[2];                      /**< Set specific mapping of lanes to connector pins
...
typedef struct {
  uint32_t               Flags;                  /**< Descriptor flags
                                                  ...
                                                  */
  PCIe_ENGINE_DATA     EngineData;               ///< Engine data
  PCIe_DDI_DATA        Ddi;                      ///< DDI port specific configuration info
} PCIe_DDI_DESCRIPTOR;
...
typedef struct {
  ...
  PCIe_DDI_DESCRIPTOR  *DdiLinkList;             ///< Pointer to array DDI link descriptors (Last element of array must be terminated with DESCRIPTOR_TERMINATE_LIST).
...
#define EXT_DISPLAY_PATH_CAPS_HDMI20_PARADE_PS175        0x0C         ///< BIT[3:2] DP -> HDMI reconverter chip
...
#define  PCIE_PORT_DATA_INITIALIZER(mPortPresent, mChannelType, mDevAddress, mHotplug, mMaxLinkSpeed, mMaxLinkCap, \
          mAspm, mResetId) \
        {mPortPresent, mChannelType, mDevAddress, 0, mMaxLinkSpeed, mAspm, mHotplug, mResetId, 0, \
         {0, mMaxLinkCap, 0, 0}, {0, 0, 0}, EndpointDetect, \
         {ADAPT_IOC_ENABLED, ADAPT_DFE_CONTROL_DISABLED, ADAPT_LEQ_CONTROL_DC_GAIN_POLE, ADAPT_DOC_DISABLED, \
          ADAPT_FOMC_ENABLED, ADAPT_PIOC_DISABLED}}
#define  PCIE_PORT_DATA_INITIALIZER_V2(mPortPresent, mChannelType, mDevAddress, mDevFunction, mHotplug, mMaxLinkSpeed, \
          mMaxLinkCap, mAspm, mResetId, mClkPmSupport) \
        {mPortPresent, mChannelType, mDevAddress, mDevFunction, mMaxLinkSpeed, mAspm, mHotplug, mResetId, 0, \
         {0, mMaxLinkCap, 0, mClkPmSupport}, {0, 0, 0}, EndpointDetect, \
         {ADAPT_IOC_ENABLED, ADAPT_DFE_CONTROL_DISABLED, ADAPT_LEQ_CONTROL_DC_GAIN_POLE, ADAPT_DOC_DISABLED, \
          ADAPT_FOMC_ENABLED, ADAPT_PIOC_DISABLED}}
#define  PCIE_PORT_DATA_INITIALIZER_GEN3(mPortPresent, mChannelType, mDevAddress, mDevFunction, mHotplug, mMaxLinkSpeed, \
          mMaxLinkCap, mAspm, mResetId, mClkPmSupport, \
          mInitOffsetCancellation, mDFEControl, mLEQControl, mDynamicOffsetCalibration, mFOMCalculation, \
          mPIOffsetCalibration) \
        {mPortPresent, mChannelType, mDevAddress, mDevFunction, mMaxLinkSpeed, mAspm, mHotplug, mResetId, 0, \
         {0, mMaxLinkCap, 0, mClkPmSupport}, {0, 0, 0}, EndpointDetect, \
         {mInitOffsetCancellation, mDFEControl, mLEQControl, mDynamicOffsetCalibration, mFOMCalculation, \
          mPIOffsetCalibration}}
...
typedef struct {
  uint32_t               Flags;                  /**< Descriptor flags
                                                  ...
                                                  */
  IOMMU_REQUESTOR_ID   RequestorIdStart;         ///< Requester ID start
  IOMMU_REQUESTOR_ID   RequestorIdEnd;           ///< Requester ID end (use same as start for single ID)
  uint64_t               RangeBaseAddress;       ///< Physical base address of exclusion range
