...
void PcieConfigEngineDebugDump (PCIe_ENGINE_CONFIG *EngineList)
{
  if (EngineList->EngineData.StartLane != 255) {
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE, "        Descriptor Flags - 0x%08x\n", EngineList->Header.DescriptorFlags);
    ...
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE, "        Scratch        - %d\n", EngineList->Scratch);
    ...
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE,
      "          Port Training - %s\n",
      (EngineList->Type.Port.PortData.PortPresent == PortDisabled) ? L"Disable" : L"Enabled"
      );
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE,
    ...
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE,
      "          Hotplug - %s\n",
      (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugDisabled) ? L"Disabled" : (
        (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugBasic) ? L"Basic" : (
          (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugServerExpress) ? L"Server" : (
            (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugEnhanced) ? L"Enhanced" : (
              (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugServerEntSSD) ? L"ServerSsd" : (
                (EngineList->Type.Port.PortData.LinkHotplug == PcieHotplugInboard) ? L"Inboard" : L"Unknown")))))
      );
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE,
      "          ASPM    - %s\n",
      (EngineList->Type.Port.PortData.LinkAspm == AspmDisabled) ? L"Disabled" : (
        (EngineList->Type.Port.PortData.LinkAspm == AspmL0s) ? L"L0s" : (
          (EngineList->Type.Port.PortData.LinkAspm == AspmL1) ? L"L1" :  (
            (EngineList->Type.Port.PortData.LinkAspm == AspmL0sL1) ? L"L0s & L1" : L"Unknown")))
      );
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE,
      "          Speed   - %d\n",
    ...
    NBIO_TRACEPOINT(SIL_TRACE_VERBOSE, "      EsmSpeedBump   - %d\n", EngineList->Type.Port.EsmSpeedBump);
  }
}
...
void PcieConfigDebugDump (PCIe_PLATFORM_CONFIG *Pcie)
{
  ...
  NBIO_TRACEPOINT(SIL_TRACE_INFO,
    "  PSPP Policy - %s\n",
    (Pcie->PsppPolicy == SilPsppPowerSaving) ? L"Power Saving" :
    (Pcie->PsppPolicy == SilPsppBalance) ? L"Balanced Mode" : (
      (Pcie->PsppPolicy == SilPsppPerformance) ? L"Performance" : (
        (Pcie->PsppPolicy == SilPsppDisabled) ? L"Disabled" : L"Unknown"))
    );
...
void PcieUserDescriptorConfigDump (PCIe_ENGINE_DESCRIPTOR *EngineDescriptor)
{
  ...
  NBIO_TRACEPOINT(SIL_TRACE_INFO,
    "  Engine Type - %s\n",
    (EngineDescriptor->EngineData.EngineType == PciePortEngine) ? L"PCIe Port" : (
      (EngineDescriptor->EngineData.EngineType == PcieDdiEngine) ? L"DDI Link" : (
        (EngineDescriptor->EngineData.EngineType == PcieUnusedEngine) ? L"Unused" : L"Invalid"))
    );
  NBIO_TRACEPOINT(SIL_TRACE_INFO,
    "    Start Phy Lane - %d\n    End   Phy Lane - %d\n",
