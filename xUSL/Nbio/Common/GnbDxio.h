...
// Check if descriptor is a PCIe engine
...
typedef struct {
  ...
  bool            BlackList;                            ///< Blacklist device
} PCIe_LINK_ASPM;
...
typedef struct {
  uint8_t NewDeviceAddress;                                ///< New PCI address (Device,Function)
  uint8_t NativeDeviceAddress;                             ///< Native PCI address (Device,Function)
} PCI_ADDR_LIST;
...
typedef struct {
  ...
  uint8_t                   CcixCoreConfig;             ///< Ccix CORE Configuration
  ...
  uint32_t                   AdvancedErrorReporting:1;        ///< Adcanced Error Reporting
