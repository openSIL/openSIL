...
typedef SIL_STATUS (*PCIe_RUN_ON_DESCRIPTOR_CALLBACK)
  (SIL_CONTEXT *SilContext, PCIe_DESCRIPTOR_HEADER *Descriptor, void *Buffer, PCIe_PLATFORM_CONFIG *Pcie);
typedef SIL_STATUS (*PCIe_RUN_ON_WRAPPER_CALLBACK)
  (SIL_CONTEXT *SilContext, PCIe_WRAPPER_CONFIG *Wrapper, void *Buffer, PCIe_PLATFORM_CONFIG *Pcie);
typedef void (*PCIe_RUN_ON_WRAPPER_CALLBACK2)
  (SIL_CONTEXT *SilContext, PCIe_WRAPPER_CONFIG *Wrapper, void *Buffer, GNB_HANDLE *GnbHandle);
typedef void (*PCIe_RUN_ON_ENGINE_CALLBACK)
  (SIL_CONTEXT *SilContext, PCIe_ENGINE_CONFIG *Engine, void *Buffer, PCIe_PLATFORM_CONFIG *Pcie);
typedef void (*PCIe_RUN_ON_ENGINE_CALLBACK2)
  (SIL_CONTEXT *SilContext, PCIe_ENGINE_CONFIG *Engine, void *Buffer, PCIe_WRAPPER_CONFIG *Wrapper);
...
GNB_HANDLE * GetGnbHandle (SIL_CONTEXT  *SilContext);
...
void PcieConfigRunProcForAllEngines (
  SIL_CONTEXT *SilContext, uint32_t DescriptorFlags, PCIe_RUN_ON_ENGINE_CALLBACK Callback,
  void *Buffer, PCIe_PLATFORM_CONFIG *Pcie
  );
void PcieConfigRunProcForAllEnginesInWrapper (
  SIL_CONTEXT *SilContext, uint32_t DescriptorFlags, PCIe_RUN_ON_ENGINE_CALLBACK2 Callback,
  void *Buffer, PCIe_WRAPPER_CONFIG *Wrapper
  );
SIL_STATUS PcieConfigRunProcForAllWrappers (
  SIL_CONTEXT  *SilContext, uint32_t DescriptorFlags, PCIe_RUN_ON_WRAPPER_CALLBACK Callback,
  void *Buffer, PCIe_PLATFORM_CONFIG *Pcie
  );
void PcieConfigRunProcForAllWrappersInNbio (
  SIL_CONTEXT *SilContext, uint32_t DescriptorFlags, PCIe_RUN_ON_WRAPPER_CALLBACK2 Callback,
  void *Buffer, GNB_HANDLE *GnbHandle
  );
