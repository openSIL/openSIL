 ...
 *
 * @details Initialize variable MTRRs and Program 0~TOM as WB.
 *
  ...
  )
{
  ...
  PhysicalAddressBits = xUslGetPhysAddrSize ();
  VariableMtrrMask = ((uint64_t)1 << PhysicalAddressBits) - 1;
  ...
  while ((CurAddr >= AMD_MTRR_VARIABLE_BASE0) && (CurAddr < AMD_MTRR_VARIABLE_BASE7) && (CurBase < Cache32bTOP)) {
    ...
    Value32 = ~Value32;
    LocalMsrRegister = VariableMtrrMask | (Value32 << 16) | BIT_64(11);
    xUslWrMsr(CurAddr, LocalMsrRegister);
...
void CcxCacheInitCommon (void) {
  ...
  xUslWrMsr(AMD_AP_MTRR_FIX64K_00000, AMD_MTRR_FIX64K_WB_DRAM);     // 0 - 512K = WB Mem
  xUslWrMsr(AMD_AP_MTRR_FIX16K_80000, AMD_MTRR_FIX16K_WB_DRAM);     // 512K - 640K = WB Mem

  xUslMsrOr(MSR_SYS_CFG, (SYS_CFG_MTRR_FIX_DRAM_EN | SYS_CFG_MTRR_VAR_DRAM_EN));
