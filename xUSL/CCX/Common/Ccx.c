...
static const CCXCLASS_DATA_BLK gCcxConfigData = {
  .CcxInputBlock = {
    ...
    .AmdReserved = CONFIG_CCX_HMKEE_ENABLE,
    .AmdReserved1 = 0xFF,
    .AmdReserved2 = 0xFF,
    .AmdReserved3 = 0xFF,
    .AmdReserved4 = 0,
    ...
    .EnSpecStFill = true,
    .EnableFSRM = CONFIG_CCX_FSRM_ENABLE,
    .EnableERMS = CONFIG_CCX_ERMS_ENABLE,
    .EnableRMSS = CONFIG_CCX_RMSS_ENABLE,
    .EnableSvmAVIC = true,
    .IbsHardwareEn = false,
    .EnableAvx512 = 0xFF,
    .EnableSvmX2AVIC = true,
    .MonMwaitDis = 0xFF,
    ...
    .AmdCpuPauseDelay = 0xFF, // See CPU_PAUSE_DELAY_AUTO
    .AmdStatisticalCorrectPredictor = false,
    .CapsuleUpdateDetected = false,
    .CpuPauseCntSel_1_0 = 0xFF, // See CPU_PAUSECNTSEL_1_0_AUTO
    .DisFstStrErmsb = 0xFF,
    .AdaptiveAlloc = 0xFF,
    .SmallHammerConfiguration = 0xFF,
    .ErmsbIntermThld = 0,
    .ErmsbRepo = 0xFF,
    .SvmEnable = true,
    .SvmLock = true,
  ...
  )
{
  ...
  Status = DfApi->DfGetSystemInfo(&NumberOfSockets, NULL, NULL, NULL, NULL);
  if (Status == SilPass) {
    Status = DfApi->DfGetProcessorInfo(0, &NumberOfDies, NULL);
  }
  if (Status != SilPass) {
    assert(Status == SilPass);
    return SilInvalidParameter;
  ...
  GetCoreTopologyOnDieMax(0, 0, &NumberOfCcds, &NumberOfComplexes, &NumberOfCores, &NumberOfThreads);
  ...
  case x2ApicMode:
    if ((CcxIommuFeatureData.Iommu_Sup == 0) || (CcxIommuFeatureData.XT_Sup == 0)) {
      ApicMode = xApicMode;
      CCX_TRACEPOINT(SIL_TRACE_INFO,
        "IOMMU disabled or has no XT_Sup, Fall back PcdAmdApicMode to 0x%X\n",
        ApicMode
        );
  ...
  case ApicCompatibilityMode:
    ...
    ApicId = CcxXfer->CalcLocalApic((NumberOfSockets - 1),
      (NumberOfDies - 1),
      (NumberOfCcds - 1),
      (NumberOfComplexes - 1),
      (NumberOfCores - 1),
  ...
  CcxXfer->StoreDowncoreCoreCount(CcxConfigData);
  ...
  ApLaunchGlobalData.ApMsrSyncList = CcxXfer->GetApMsrSyncList ();

  // Load microcode on the BSP
  if (CcxConfigData->CcxInputBlock.CcxLoadUcodePatch) {
    LoadMicrocodePatch((uint64_t *) &ApLaunchGlobalData.UcodePatchAddr);
  }
  ...
  CcxProgramTablesAtReset(&ApLaunchGlobalData,
    CcxConfigData
    );
  ...
  CcxEnableWdt((CORE_WATCHDOG *) &ApLaunchGlobalData.CpuWdt);

  // Enable SMEE
  CcxEnableSmeeHmkee(CcxConfigData->CcxInputBlock.AmdSmee, CcxConfigData->CcxInputBlock.AmdReserved);
  ...
  ApLaunchGlobalData.SetCacWeightsEnable = (CcxGetCacWeights((uint64_t *) &ApLaunchGlobalData.CacWeights[0]) ==
    SilPass) ? true : false;
  ...
  for (Socket = 0; Socket < NumberOfSockets; Socket++) {
    Status = DfApi->DfGetProcessorInfo(Socket, &NumberOfDies, NULL);
    if (Status != SilPass) {
      NumberOfDies = 0;
    }
    CCX_TRACEPOINT(SIL_TRACE_INFO, "    Socket %d has %d Die(s)\n", Socket, NumberOfDies);
    for (Die = 0; Die < NumberOfDies; Die++) {
      ApobIp2IpApi->ApobGetCcdLogToPhysMap(Socket, Die, &ApobCcdLogToPhysMap);
      Status = GetCcdCountOnDie(&ApobCcdLogToPhysMap, Socket, Die, &NumberOfCcds);
      if (Status != SilPass) {
        NumberOfCcds = 0;
      }
      CCX_TRACEPOINT(SIL_TRACE_INFO, "    Socket %d Die %d has %d CCD(s)\n", Socket, Die, NumberOfCcds);
      for (Ccd = 0; Ccd < NumberOfCcds; Ccd++) {
        Status = GetComplexCountOnCcd(&ApobCcdLogToPhysMap, Socket, Die, Ccd, &NumberOfComplexes);
        if (Status != SilPass) {
          NumberOfComplexes = 0;
        }
        CCX_TRACEPOINT(SIL_TRACE_INFO,
        ...
        for (Ccx = 0; Ccx < NumberOfComplexes; Ccx++) {
          Status = GetCoreCountOnComplex(&ApobCcdLogToPhysMap, Socket, Die, Ccd, Ccx, &NumberOfCores);
          if (Status != SilPass) {
            NumberOfCores = 0;
          }
          CCX_TRACEPOINT(SIL_TRACE_INFO,
          ...
          for (Core = 0; Core < NumberOfCores; Core++) {
            ...
            for (Thread = 0; Thread < NumberOfThreads; Thread++) {
              if (!((Socket == 0) && (Die == 0) && (Ccd == 0) && (Ccx == 0) && (Core == 0) && (Thread == 0))) {
                ...
                ApNumBfLaunch++;
                SmuApi->SmuLaunchThread(Socket, Die, Ccd, Ccx, Core, Thread);
  ...
  xUslMsrOr(SIL_RESERVED_0387, TW_CFG_COMBINED_CR0_CD);
  ...
  )
{
  ...
  LclCcxDataBlk = (CCXCLASS_DATA_BLK *) xUslFindStructure(SilId_CcxClass, 0);
  CCX_TRACEPOINT(SIL_TRACE_INFO,
  ...
  Status = InitializeCcxAndLaunchAps(LclCcxDataBlk, CcxRegTableListAtGivenTP);
  return Status;
}
  ...
  )
{
  ...
  if (IsSmtDisabled || !xUslIsComputeUnitPrimary ()) {
    xUslMsrOr(SIL_RESERVED_0387, BIT_64(49));
  }
}
  ...
  )
{
  ...
  CcxProgramTablesAtReset(ApLaunchGlobalData, NULL);
  ...
  )
{
  ...
  if (CcxDataConfigData->CcxInputBlock.AmdSnpMemCover != SNP_DISABLE) {
    ...
    CCX_TRACEPOINT(SIL_TRACE_INFO, "VMPL Enable\n");
    ...
    CcxXfer->CalcRMPTableBase(&RmpTableSize,
      &RmpTableBase,
      &SnpMemSizeToCover,
