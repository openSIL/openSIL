  ...
  )
{
  ...
  if (InstanceId == SMN_ON_ALL_DIES) {
    ...
    for (DieLoop = 0; DieLoop < SystemDieNumber; DieLoop++) {
      SmuApi->SmuRegisterRMWDie(DieLoop,
        RegisterIndex,
        RegisterANDValue,
        RegisterORValue
        );
    ...
    SmuApi->SmuRegisterRMWDie(InstanceId,
      RegisterIndex,
      RegisterANDValue,
      RegisterORValue
      );
  ...
  )
{
  ...
  if ((SmuIndexEntry->Index == SMUSVI0_PLANE1_LOAD_INDEX) ||
    (SmuIndexEntry->Index == SIL_RESERVED_0003) ||
  ...
  )
{
  ...
  if (DoesEntryTypeSpecificInfoMatch(Criteria->ProfileFeats.ProfileValue,
    ...
    SetSmuIndexEntry(Criteria, (uint8_t **) &SmuIndexEntry);
  }
  // Entry MUST point to next register entry
  ...
  )
{
  ...
  if (Source->MSB >= Source->LSB) {
    ...
    assert(false);// Logic says these asserts are unnecessary
