  ...
  )
{
   ...
   */
  if (SilGetCommon2RevXferTable(SilContext, SilId_CxlClass, (void **)(&CxlXferTable)) != SilPass) {
    return;
  }
  ...
  )
{
   ...
   * Get IP block data
   */
  SilData = (CXLCLASS_DATA_BLK *)xUslFindStructure(SilContext, SilId_CxlClass, 0);
  if (SilData == NULL) {
  ...
  CxlPortInit(SilContext, &NbPcieData->PciePlatformConfig);
 ...
 */
SIL_STATUS CxlClassSetInputBlock (
  SIL_CONTEXT  *SilContext
  )
{
  CXLCLASS_DATA_BLK *CxlConfigData;

  CxlConfigData = (CXLCLASS_DATA_BLK *)SilCreateInfoBlock(SilContext,
    SilId_CxlClass,
