  ...
  )
{
  ...
  if (PcieCapPtr != 0) {
    Value = xUSLPciRead8(Device.AddressValue | (PcieCapPtr + 0x2));
    return Value >> 4;
  }
 ...
 *
 * @details Reads CXL register and output its address
 *
  ...
  )
{
  ...
  CXL_TRACEPOINT(SIL_TRACE_INFO, "CxlRegisterRead: %08x\n", RegisterAddress);
  ...
  )
{
  ...
  while (DvsecPtr != 0) {
    xUSLPciRead(Address | (DvsecPtr + DVSEC_VID_OFFSET),
      AccessWidth32,
      &DvsecValue
      );

    if (((uint16_t) DvsecValue == DvsecVendorId) || ((uint16_t) DvsecValue == DvsecVendorId2)) {
      xUSLPciRead(Address | (DvsecPtr + DVSEC_ID_OFFSET),
        AccessWidth16,
        &DvsecValue
        );
  ...
  )
{
  ...
  RegisterAddress = (uint32_t *) ((uintptr_t) DsRcrb + RegOffset);
  CXL_TRACEPOINT(SIL_TRACE_INFO, "CxlRegisterWrite: %08x\n", RegisterAddress);
  ...
  )
{
  ...
  CXL_TRACEPOINT(SIL_TRACE_INFO,
    "MAIN_FLOW: Begin to allocate bars for SMN low %x high %x, size %x\n",
    MmioBarLow,
    MmioBarHigh,
    MemorySize
    );
  ...
  CXL_TRACEPOINT(SIL_TRACE_INFO,
    "MAIN_FLOW: FabricAllocateMmio : Socket %d , RB # %d\n",
  ...
  )
{
  ...
  SIL_RESERVED_UNION_0041            Rcrb;

  Rcrb.RcrbPointer = NULL;
  Rcrb.field0 = Engine->Type.Cxl.UsRcrb;
  ...
  for (Index = 0x10; Index <= 0x24; Index += 4) {
    ...
    Bar32.Value = CxlRegisterRead(Rcrb.RcrbPointer, Index);
    if (Bar32.Field.MemIo == 0) {
      if (Bar32.Field.MemSize == MEM_32_BAR) {
        ...
        CXL_TRACEPOINT(SIL_TRACE_INFO, " 32 bit bar at 0x%x with size 0x%lX\n", Index, Value);
        Mem32Size += Value;
  ...
  )
{
  ...
  while (GnbHandle != NULL) {
    if (GnbHandle->RBIndex < Pcie->RootBridgesPerSocket) {

      Resources.BusLimits[GnbHandle->InstanceId] = (uint8_t) GnbHandle->BusNumberLimit;

      CXL_TRACEPOINT(SIL_TRACE_INFO,
        "  inst/bus/limit/rbidx/lrbidx: %d/%02x/%02x/%02x/%02x\n",
  ...
  CxlXferTable->CxlDevListGenerate(Pcie);
