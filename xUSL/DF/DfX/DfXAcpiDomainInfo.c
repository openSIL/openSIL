  ...
  )
{
  ...
  if ((ApobIp2IpApi->ApobAmdGetApobEntryInstance(SilContext,
    APOB_FABRIC,
  ...
  )
{
  ...
  DomainInfo = (DF_DOMAIN_INFO_BLK *) xUslFindStructure(SilContext, SilId_DfClass, DF_DOMAIN_INSTANCE);
  if (DomainInfo == NULL) {
  ...
  NumberOfDies = (uint32_t) DfXfer->DfGetNumberOfSystemDies(SilContext);
  ...
  SystemCxlCount = BuildCxlInfo(SilContext, NumberOfPhysicalDomains, DomainInfo);
  ...
  if (CcxAsNuma) {
    ...
    ReportedIndex = 0;
    for (Socket = 0; Socket < NumberOfCpus; Socket++) {
      if (DomainInfo->CcdCount[Socket] <= PROJ_MAX_CCD_DIES_PER_SOCKET) {
        for (Ccd = 0; Ccd < DomainInfo->CcdCount[Socket]; Ccd++) {
          if (DomainInfo->CcxPerCcd[Socket] <= PROJ_MAX_COMPLEXES_PER_CCD) {
            for (Ccx = 0; Ccx < DomainInfo->CcxPerCcd[Socket]; Ccx++) {
              ...
              DomainInfo->ReportedDomainInfo[ReportedIndex].SocketMap
                = GetSocketMapFromNormalizedCsMap(SilContext,
                DomainInfo->PhysicalDomainInfo
  ...
  )
{
  ...
  CcxPerCcd = SilVars->ActiveSoC.NumCcxPerCcd;
  DomainInfo = (DF_DOMAIN_INFO_BLK *) xUslFindStructure(SilContext, SilId_DfClass, DF_DOMAIN_INSTANCE);
  ...
  if (Status != SilPass) {
    ...
    assert(Socket < DfXfer->DfGetNumberOfProcessorsPresent(SilContext));
    ...
    assert(Die < DfGetNumberOfDiesOnSocket(SilContext, Socket));
    Status = SilInvalidParameter;
