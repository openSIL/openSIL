  ...
  for (Socket = 0; Socket < SocketCount; Socket++) {
    ...
    for (Die = 0; Die < DieCount; Die++) {
      ...
      for (RB = 0; RB < RBCount; RB++) {
        PciLimit = DfXfer->DfGetHostBridgeBusLimit(SilContext, Socket, Die, RB);
  ...
  )
{
  ...
  Status = SilGetCommon2RevXferTable(SilContext, SilId_DfClass, (void **) &DfXfer);
  assert(Status == SilPass);
  return (DfXfer->DfGetNumberOfSystemDies(SilContext) / DfXfer->DfGetNumberOfProcessorsPresent(SilContext));
}
  ...
  )
{
  ...
  if (Socket >= DfXfer->DfGetNumberOfProcessorsPresent(SilContext)) {
    return SilInvalidParameter;
  }

  if (Die >= DfGetNumberOfDiesOnSocket(SilContext, Socket)) {
    return SilInvalidParameter;
  }

  if (Index >= DfXfer->DfGetNumberOfRootBridgesOnDie(SilContext, Socket, Die)) {
    return SilInvalidParameter;
  }

  if (SystemFabricID != NULL) {
    *SystemFabricID = DfGetHostBridgeSystemFabricID(SilContext, Socket, Die, Index);
  }

  if (BusNumberBase != NULL) {
    *BusNumberBase = DfXfer->DfGetHostBridgeBusBase(SilContext, Socket, Die, Index);
  }

  if (BusNumberLimit != NULL) {
    *BusNumberLimit = DfXfer->DfGetHostBridgeBusLimit(SilContext, Socket, Die, Index);
  }
  ...
  if (HasFchDevice != NULL) {
    *HasFchDevice = DfXfer->DfHasFch(SilContext, Socket, Die, Index);
  }

  if (HasSystemMgmtUnit != NULL) {
    *HasSystemMgmtUnit = DfXfer->DfHasSmu(SilContext, Socket, Die, Index);
  }
  ...
  )
{
  ...
  if (NumberOfRootBridges != NULL) {
    *NumberOfRootBridges = DfXfer->DfGetNumberOfRootBridgesOnDie(SilContext, Socket, Die);
  }
  ...
  )
{
  ...
  if (SocketIndex >= DfXfer->DfGetNumberOfProcessorsPresent(SilContext)) {
    return SilInvalidParameter;
  }

  if (DieCount != NULL) {
    *DieCount = (uint32_t) DfGetNumberOfDiesOnSocket(SilContext, SocketIndex);
  }

  if (RootBridgeCount != NULL) {
    *RootBridgeCount = (uint32_t) DfXfer->DfGetNumberOfRootBridgesOnSocket(SilContext, SocketIndex);
  }
  ...
  )
{
  ...
  if (NumberOfInstalledProcessors != NULL) {
    *NumberOfInstalledProcessors = DfXfer->DfGetNumberOfProcessorsPresent(SilContext);
  }

  if (TotalNumberOfDie != NULL) {
    *TotalNumberOfDie = DfXfer->DfGetNumberOfSystemDies(SilContext);
  }

  if (TotalNumberOfRootBridges != NULL) {
    *TotalNumberOfRootBridges = DfXfer->DfGetNumberOfSystemRootBridges(SilContext);
  }

  if (SystemFchRootBridgeLocation != NULL) {
    DfXfer->DfGetSystemComponentRootBridgeLocation(SilContext,
      PrimaryFch,
