...
#define SIL_RESERVED_0001        2
#define SIL_RESERVED_0002  128
...
typedef struct {
  ...
  bool                  IsRmpAvailable;                   ///< Is RMP available
  uint16_t              NumberOfProfiles;                 ///< Number of RMP Profiles
  APOB_RMP_PROFILE_DATA Profiles[SIL_RESERVED_0001];
  uint8_t               RmpSpdData[SIL_RESERVED_0002];
...
typedef struct {
  uint32_t Type : 8;          ///< [0:7], Type of BIOS entry
  uint32_t field_bit_0 :1;       ///< [8], Flag to identify the entry need
                              ///< to reload during S3 exit
  uint32_t field_bits_1_to_7 :7;      ///< [9:15], Reserved
  uint32_t field_bit_8: 1; ///< [16], Set for SEC or EL3 fw, which
                              ///< will be authenticate by PSP FW known as HVB
  uint32_t field_bit_9: 1;           ///< [17], Copy: 1- copy BIOS image from source
                              ///< to destination 0- Set region attribute
                              ///< based on <ReadOnly, Source, size> attributes
  uint32_t field_bit_10 : 1;      ///< [18], 1: Set region to read-only
                              ///< (applicable for ARM- TA1/TA2)
                              ///< 0: Set region to read/write
  uint32_t field_bit_11 : 1;    ///< [19], 1: Compressed
  uint32_t field_bits_12_to_15 : 4;      ///< [20:23], Specify the Instance of an entry
  uint32_t field_bits_16_to_18 : 3;    ///< [24:26], Specify the SubProgram
  uint32_t field_bits_19_to_20:2;           ///< [27:28], Specify the RomId
  uint32_t field_bit_21 : 1;      ///< [29], 1: Region writable, 0: Region read only
  uint32_t Reserved : 2;      ///< [30:31], Reserve for future use
} SIL_RESERVED_STRUCT_0043;
...
typedef struct {
  ...
  uint8_t     PhysNbioMap;     ///< Bit n set means NBIO n on the given socket
                               ///< should be included in this region
  uint8_t     Alignment;       ///< Bit position to align to
                               ///< (e.g. 32 will result in the region being 4GB aligned)
