 ...
 * @param  CcxIndex             used access ComplexMap
 * @param  PhysCcdNumber        PhysComplexNumber
 *
 ...
 *                              for initializing the SIL_CONTEXT structure.
 * @param  ApobInstanceId
 * @param  CcxIndex              used to access ComplexMap
 * @param  PhysComplexNumber
 *
 * @retval       SilPass           Function succeed
 * @retval       NON-ZERO          Error occurs
 */
 ...
 *                              for initializing the SIL_CONTEXT structure.
 * @param ApobInstanceId
 * @param CcxIndex             Index to access ComplexMap
 * @param CoreIndex            Index to access CoreInfo
 * @param PhysCoreNumber
 *
 *  @retval       SilPass           Function succeed
 *  @retval       NON-ZERO          Error occurs
 */
 ...
 * @param  CcxIndex             Index to access ComplexMap
 * @param  CoreIndex            Index to access CoreInfo
 * @param  ThreadIndex          Index to access IsThreadEnabled
 * @param  IsThreadEnabled
 *
 *  @retval       SilPass           Function succeed
 *  @retval       NON-ZERO          Error occurs
 */
...
ConvertSocCcdLogToPhysMap (
  ...
  APOB_CCD_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_PHX *SocCcdLogToPhysMap
  )
{
  uint8_t             CcdLoop;
  uint8_t             ComplexLoop;
  ...
  ApobGetMaxDieInfoPhx(SilContext, &SocMaxDieInfo);
  ...
  for (CcdLoop = 0; CcdLoop < SocMaxDieInfo.MaxSocCcdsPerDieValue; CcdLoop++) {
    CommonCcdLogToPhysMap->CcdMap[CcdLoop].PhysCcdNumber = SocCcdLogToPhysMap->CcdMap[CcdLoop].field0;
    ...
    for (ComplexLoop = 0; ComplexLoop < SocMaxDieInfo.MaxSocCcxPerCcdValue; ComplexLoop++) {
      CommonCcdLogToPhysMap->CcdMap[CcdLoop].ComplexMap[ComplexLoop].PhysComplexNumber =
        SocCcdLogToPhysMap->CcdMap[CcdLoop].field1[ComplexLoop].PhysComplexNumber;
      ...
      for (CoreLoop = 0; CoreLoop < SocMaxDieInfo.MaxSocCoresPerComplexValue; CoreLoop++) {
        CommonCcdLogToPhysMap->CcdMap[CcdLoop].ComplexMap[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber =
          SocCcdLogToPhysMap->CcdMap[CcdLoop].field1[ComplexLoop].CoreInfo[CoreLoop].PhysCoreNumber;
        ...
        for (ThreadLoop = 0; ThreadLoop < SocMaxDieInfo.MaxSocThreadPerCore; ThreadLoop++) {
          ...
          CoreInfo[CoreLoop].IsThreadEnabled[ThreadLoop] =
            SocCcdLogToPhysMap->CcdMap[CcdLoop].field1[ComplexLoop].
            CoreInfo[CoreLoop].IsThreadEnabled[ThreadLoop];
        }
      }
    }
  ...
  )
{
  ...
  APOB_TYPE_HEADER                                 *ApobEntry;
  SIL_STATUS                                       Status;
  APOB_CCD_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_PHX *PhxApobEntry;
  ...
  PhxApobEntry = (APOB_CCD_LOGICAL_TO_PHYSICAL_MAP_TYPE_STRUCT_PHX *)ApobEntry;
  ...
  ConvertSocCcdLogToPhysMap(SilContext, ApobCcdLogToPhysMap, PhxApobEntry);
 ...
 *                              can be passed to openSIL. The host firmware is responsible
 *                              for initializing the SIL_CONTEXT structure.
 * @param ActionOnBistFailure   Bist Failure Action
 *
 *
...
{
  uint32_t            MaxPhxCpusValue;
  APOB_SOC_DIE_INFO   SocMaxDieInfo;

  ApobGetMaxDieInfoPhx(SilContext, &SocMaxDieInfo);

  MaxPhxCpusValue = (uint32_t)( SocMaxDieInfo.MaxSocSocketsSupportedValue *
  ...
  )
{
  ...
  ApobSocDieInfo->MaxSocThreadPerCore = CCX_MAX_THREADS_PER_CORE;
  ApobSocDieInfo->MaxSocCcdPerSocket = MAX_CCDS_PER_DIE * CCX_MAX_DIES_PER_SOCKET;
}
