 ...
 *
 * @param [in] GnbHandle     Pointer to silicon descriptor
  ...
  )
{
  ...
  if (AskInstanceCount != 0) {
    ...
    if (AncillaryDataSize != 0) {
      ...
      TotalSize = InitGlobalAncillaryData(GnbHandle, SocketTopology, AncillaryData);
      ...
      if (TopologyEntry->EngineData.EngineType != MpioUnusedEngine) {
        if (MpioTranslateLanes(GnbHandle,
          ...
          case MpioUSB_OVER_PCIE:
            Usb4LaneNumberCfg(Counter, AskEntry, TopologyEntry);
