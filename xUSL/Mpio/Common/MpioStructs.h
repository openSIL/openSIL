...
typedef enum {
  ...
  MpioEthernetEngine = 0x10,                              /**< Ethernet (GBe, XGBe)
                                                           *   @li __Deprecated__
                                                           */
  ...
  MpioHDaudioEngine,                                      //  For NBIF HDaudio Enable (internal use only)
...
/**
 * @brief High level data structures for passing topology
...
typedef struct {
  ...
  uint8_t        MpioEndLane;              /**< Internal coding of end lane
                                            */
  uint8_t        SearchDepth;              /**< SearchDepth only uses 1 bit - always initialize to 0 will be updated
                                            * dynamically
                                            */
} MPIO_ENGINE_DATA;
 ...
 */
typedef struct  {
  uint8_t        LinkComplianceMode     :1;  /**< Force port into compliance mode (device will not be trained, port
                                              *  output compliance pattern)
                                              */
  uint8_t        LinkSafeMode           :1;  /**< Safe mode PCIe capability. (Parameter may limit PCIe speed requested
                                              *  through MPIO_PORT_DATA::LinkSpeedCapability)
                                              *  @li @b 0 - port can advertise maximum supported capability
                                              *  @li @b 1 - port limit advertised capability & speed to PCIe Gen1
                                              */
  ...
  uint8_t        ClkPmSupport           :1;  /**< Clock Power Management Support
                                              ...
                                              *  @li @b 1 - Clock Power Management configured according to PCIe
                                              *  device capability
                                              */
...
typedef struct  {
  ...
  uint8_t                   DeviceNumber        :5;  /**< PCI Device number for port.
                                                      ...
                                                      *   @li @b N - Port device number (See available
  ...
  uint8_t                   AutoSpdChng         :2;  /**< Upstream Auto Speed Change Allowed/
                                                      *  @li @b 0 - Use default implementation
                                                      *             (Disabled for Gen1, Enabled for Gen2/3)
  ...
  uint8_t                   SrisAutodetectFactor:2;    /**< Controls the multiplier for SKP ordered set interval when
                                                        *  generated based on elasticity buffer pointer slip
                                                        *  feedback from PCS
                                                        */
...
typedef struct {
  uint32_t         size;                                       ///< Size in bytes of override table
  uint8_t          data[MPIOTEMP_MAX_SIZE_OVERRIDE_TABLE];     ///< Override table of maximum size
} MPIO_PHYOVR_TABLE;
...
typedef struct {
  ...
  uint32_t               Value;               /**< Ancillary Data Value.
                                               ...
                                               *  Continue define SPD_OVRD type for each gen speed override.
                                               *    First for Gen1, Second for Gen2, Third for Gen3, etc.
                                               *  Bits [ 0: 2] Enable Mask: This mask allows the user to selectively
                                               *               override only some attributes.
                                               *               Bit0: Enable C0 override.
                                               *               Bit1: Enable Cn Override.
                                               *               Bit2: Enable Cp override.
...
typedef enum {
  ...
  MPIO_PP_DFC_EVENT,                /**< (UINT8) DFC Event for this port
                                     *   Valid values are: 0 = BIOS_EVENT_DEVICE_DISCONNECTED ,
                                                      1 = BIOS_EVENT_DEVICE_CONNECTED,
                                                      2 = BIOS_EVENT_DEVICE_NOT_PRESENT
                                     */
...
typedef struct  {
  uint8_t                ConnectorType;             /**< Display Connector Type
                                                     ...
                                                     *  @li @b 3 - Dual  Link DVI-D (see @ref
                                                     *             DualLinkDviDescription "Example Dual Link DVI
                                                     ...
                                                     *  @li @b 9 - Native CRT
                                                     *  @li @b 10 - Native LVDS
                                                     *  @li @b 11 - Auto detect LCD panel connector type. VBIOS
                                                     *              is able to auto detect the LVDS connector
                                                     *              type: native LVDS, eDP or DP-to-LVDS
                                                     *              The auto detection method only support panel
                                                     *              with EDID.
                                                     */
  ...
  MPIO_CONN_CHANNEL_MAPPING Mapping[2];             /**< Set specific mapping of lanes to connector pins
                                                     *  @li Mapping[0] define mapping for group of 4 lanes
                                                     *    starting at PCIe_ENGINE_DATA.StartLane
                                                     *  @li Mapping[1] define mapping for group of 4 lanes
                                                     *    ending at PCIe_ENGINE_DATA.EndLane (only applicable for
                                                     *    Dual DDI link)
  ...
  uint8_t                Flags;                     /**< Capabilities flags
                                                     *  @li Flags bit[0] DDI_DATA_FLAGS_DP1_1_ONLY Selects
                                                                 downgrade PHY link to DP1.1
...
typedef struct {
  ...
  MPIO_PORT_DESCRIPTOR   *PciePortList;            /**< Pointer to array of PCIe port descriptors or NULL (Last element
                                                    *  of array must be terminated with DESCRIPTOR_TERMINATE_LIST)
                                                    */
