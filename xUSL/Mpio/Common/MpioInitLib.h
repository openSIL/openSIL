...
typedef enum {
  ...
  /*! Provides the MPIO with the I2C topography information necessary to enumerate the UBM, U.3, and OCP controllers.
     ...
     This can take some time allowing the BIOS to continue on and check back with the MPIO for completion.
     ...
     @arguments
     - Argument 0 - The MPIO accessible location of the I2C data structure array containing the I2C topography for the
                 system.
  ...
  /*! Request from the BIOS to return the nth DFC connection information for a UBM connected device connector.
     ...
     @response
     - Response Register 0 = Returns the total number of device connectors associated with the I2C connection from this
                          request.
  ...
  /*! The MPIO signals an event via interrupt to the BIOS indicating that it has detected a change has been detected to
     ...
     @response
     The response provided is based on the type of node that triggered the event. The BIOS is expected to know this based
  ...
  /*!
     This command is used to respond to interrupts by setting the proper values in the argument register formatted using
     the GpioIntrCommand_t structure.
     @arguments
     - Argument Register 0 - This is the structure defined in GpioIntrCommand_t.
     @response
     - Response Register 0 - The response is always @ref SMC_Result_OK as there is no invalid value.
   */
  ...
  /*! This interface is for BIOS to pass the details of the devices to be used for external FSDL functions during s0i3.
     @arguments
     - Request 0 - BusCount
     - Request 1:6 - BusInfo
     @response
     - Response Register 0 = BusCount.
     - Error Register = @ref MSG_ACCEPTED
   */
  BIOS_MPIO_MSG_SET_EXT_PCIE_BUSES = 0x25,
  /*! Command to read/write to private PCIe addresses. These values are verified to be valid and an
     error is returned if an invalid access is requested.

     If argument 1(mask) is zero then the register is read and returned in response register 1.

     If argument 1(mask) is non-zero, then the data in argument register 2(data) uses the following logic to set only the
     masked bits:
     result = (data & mask) | (current_value & ~mask)

     @arguments
     - Argument Register 0 - SMN address for this request.
     - Argument Register 1 - Mask value for the access
      @li 0x00000000 - No write performed but read data returned in response register 1.
      @li all others - Write is performed as a RMW using this mask and result is returned in response register 1.
     - Argument Register 2 - Write Data, unused if mask is argument register1 is zero.
     @response
     - Response Register 0 - Error reported.
     - Command Register[7:0] - @ref MSG_ACCEPTED
     - Response Register 1 - Read data or result of the final write data. */
...
typedef struct {
  ...
  uint32_t    gen6EqSearchMode   :2;
  uint32_t    gen6EqSearchModeEn :2;
...
/// Cxl Device Info - Event structure to respond to initial connect info during enumeration and hotplug events
typedef struct
{
  /*
   ...
   * used in this case.
   */
  uint8_t hfc_idx;    ///< HFC index to match to BIOS database.
...
typedef struct {
  uint8_t OcpPresentStart;
  uint8_t OcpNumHosts      : 4;
  uint8_t OcpNumSockets    : 4;
  union {
    struct {
      uint8_t OcpBifPrim   : 3;
      uint8_t OcpBifSec    : 3;
      uint8_t OcpFormFactor: 1;
      uint8_t Reserved     : 1;
    } Field;
    uint8_t Identifier;
  } OcpForm;
...
typedef struct {
  ...
  struct I2C_EXPANDER {
    ...
    uint8_t Type:7;       ///< The type of this GPIO expander and is one of the following values:
                          ///<  - UBM_GPIO_EXPANDER_TYPE_9539
                          ///<  - UBM_GPIO_EXPANDER_TYPE_9535
                          ///<  - UBM_GPIO_EXPANDER_TYPE_9506
    uint8_t ClrAllInts:1; ///< Used to force MPIO to clear all interrupts on an expander in cases where additional
...
typedef struct {
  ...
  uint8_t OcpDefValid;
  uint8_t OcpDefPrimPrsntb;
  uint8_t OcpDefSecPrsntb;
  uint8_t OcpSlotNum;
