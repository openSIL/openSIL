  ...
  )
{
  ...
  while (TopologyEntry != NULL) {
    ...
    if (TopologyEntry->EngineData.EngineType == MpioPcieEngine) {
      if ((TopologyEntry->EngineData.StartLane >= GnbHandle->StartLane) &&
        (TopologyEntry->EngineData.StartLane <= GnbHandle->EndLane) &&
        (TopologyEntry->EngineData.MpioStartLane == AskEntry->desc.startLaneIndex)) {
        break;
      }
    }
  ...
  )
{
   ...
   */
  if ((SilDataCommon->SrisEnableMode != 0xFF) && ((SilDataCommon->SrisCfgType & 0x7) == 0)) {
    Engine->Type.Port.SrisEnableMode = SilDataCommon->SrisEnableMode;
    Engine->Type.Port.SrisSkipInterval = SilDataCommon->SrisSkipInterval;
    Engine->Type.Port.LowerSkpOsGenSup = SilDataCommon->SrisLowerSkpOsGenSup;
    Engine->Type.Port.LowerSkpOsRcvSup = SilDataCommon->SrisLowerSkpOsRcvSup;
  }

  if (TopologyEntry->Port.SrisEnableMode && ((SilDataCommon->SrisCfgType & 0x7) == 0)) {
  ...
  if (SilDataCommon->SrisCfgType & 0x7) {
    Engine->Type.Port.SrisEnableMode = TopologyEntry->Port.SrisEnableMode;
  }

  if (SilDataCommon->SrisCfgType & 0x6) {
    Engine->Type.Port.SrisSkipInterval = TopologyEntry->Port.SrisSkipInterval;
    Engine->Type.Port.LowerSkpOsGenSup = TopologyEntry->Port.LowerSkpOsGenSup;
    Engine->Type.Port.LowerSkpOsRcvSup = TopologyEntry->Port.LowerSkpOsRcvSup;
  }
   ...
   * Default config for all port
   */
  if ((SilDataCommon->SrisAutoDetectMode != 0xFF) && ((SilDataCommon->SrisCfgType & 0x8) == 0)) {
    Engine->Type.Port.SrisAutoDetectMode = SilDataCommon->SrisAutoDetectMode;
    Engine->Type.Port.SrisSkpIntervalSel = SilDataCommon->SrisSkpIntervalSel;
    Engine->Type.Port.SrisAutodetectFactor = SilDataCommon->SrisAutodetectFactor;
  }

  if (TopologyEntry->Port.SrisAutoDetectMode && ((SilDataCommon->SrisCfgType & 0x8) == 0)) {
  ...
  if (SilDataCommon->SrisCfgType & 0x8) {
    Engine->Type.Port.SrisAutoDetectMode = TopologyEntry->Port.SrisAutoDetectMode;
    Engine->Type.Port.SrisSkpIntervalSel = TopologyEntry->Port.SrisSkpIntervalSel;
    Engine->Type.Port.SrisAutodetectFactor = TopologyEntry->Port.SrisAutodetectFactor;
  }
  ...
  )
{
   ...
   */
  if (TopologyEntry->EngineData.StartLane <= TopologyEntry->EngineData.EndLane) {
    ...
    Engine->EngineData.EndLane = (uint16_t) TopologyEntry->EngineData.StartLane;
    Engine->EngineData.StartLane = (uint16_t) TopologyEntry->EngineData.EndLane;
  }
  ...
  Engine->Type.Port.PortCapabilities.DataLinkFeature |= ((TopologyEntry->Port.Gen4Features & BIT_8(1))? 2 : 0);
  Engine->Type.Port.PortFeatures.DataLinkFeatureExchangeControl &=
    ((TopologyEntry->Port.Gen4Features & BIT_8(0))? 0 : 1);
  ...
  SrisPlatformConfig(SilDataCommon, TopologyEntry, Engine, AskEntry);
 ...
 * @param   ThisNode             Pointer to the platform descriptor for this node
 *
 * @returns Nothing
 * @retval Nothing
 **/
  ...
  )
{
  ...
  if (SilPass == Status) {
    ...
    PortDevMap = SilDataCommon->PortDevMap;
    ...
    for (Index = 0; Index < MpioData->MpioAskCount; Index++) {
      if (AskEntry->desc.ctrlType == ASK_TYPE_PCIe) {
        ...
        if (TopologyEntry != NULL) {
          ...
          if (Engine != NULL) {
            MpioInitEngineStruct(SilDataCommon, TopologyEntry, Engine, AskEntry);
            Handle = (GNB_HANDLE *) (NbioIp2Ip->PcieConfigGetParent(DESCRIPTOR_SILICON, &(Engine->Header)));
            PcieMapPortPciAddress(SilContext, Engine, PortDevMap + (Handle->RBIndex * PortDevSize));
            Engine->Type.Port.Address.AddressValue = MAKE_SBDFO(Handle->Address.Address.Segment,
              Handle->Address.Address.Bus,
              Engine->Type.Port.PortData.DeviceNumber,
              Engine->Type.Port.PortData.FunctionNumber,
              0
              );
            if (Engine->Type.Port.IsBmcLocation && (SilDataCommon->AmdMCTPEnable)) {
              SilDataCommon->AmdMCTPMasterSeg = (uint8_t) Handle->Address.Address.Segment;
              SilDataCommon->AmdMCTPMasterID = (uint16_t)((Handle->Address.Address.Bus << 8)
    ...
    while (Handle != NULL) {
      if (Handle->SocketId == GnbHandle->SocketId) {
        ...
        while (Engine != NULL) {
          SyncEarlyTrainRemap(SilContext, Handle, PortDevMap + (Handle->RBIndex * PortDevSize), Engine);
