 ...
 * @param   PortPointer      Pointer to the platform BIOS supplied platform configuration
 *
...
/**
 * Function to calculate the ancillary data size for a PCIe engine
 *
...
/**
 * Function to populate the ancillary data for a PCIe engine
 *
 * @param GnbHandle        Pointer to the Silicon Descriptor for this node
 * @param AncillaryData    Pointer to next available location in ancillary data
 * @param PortList         Pointer to the platform BIOS supplied platform configuration
 *
 * @retval      uint32_t           Total size for this ancillary data
 *
 */
  ...
  )
{
  ...
  if (AncillaryDataSize != 0 && DWData != NULL) {
    MPIO_TRACEPOINT(SIL_TRACE_INFO,
      "MPIO Socket:%d AncData Dump Addr:0x%08X size:0x%X\n",
      GnbHandle->SocketId,
      AncillaryData,
      AncillaryDataSize
      );
    ...
    for (i = 0; i < (AncillaryDataSize / 4); i++) {
      if (!(i % 4)) {
        MPIO_TRACEPOINT(SIL_TRACE_RAW, "\n-0x%02x-", i);
      }
      MPIO_TRACEPOINT(SIL_TRACE_RAW, " %08X", DWData[i]);
    }
    MPIO_TRACEPOINT(SIL_TRACE_RAW, "\n");
  }
}
