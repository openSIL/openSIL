...
typedef struct {
  ...
  uint16_t    AmdAllowCompliance;               ///< Compliance settings
  ...
  bool        LcMultAutoSpdChgOnLastRateEnable;
  ...
  FW_ASK      AskBuffer[PROJ_MAX_SOCKETS_SUPPORTED];
  ...
  bool        SbrBrokenLaneAvoidanceSup;
  bool        AutoFullMarginSup;
  ...
  bool        AmdFabricSdxi;                  ///< Enable SDXI feature.
  bool        DataObjectExchange;             ///< Enables or disables Data Object Exchange (DOE) capability
  bool        AmdHotPlugForceSFIStrap;        ///< Enable SFI features in non-SFI handling mode
  ...
  uint16_t    CxlQosHighLimit;
  uint16_t    CxlQosTimerLimit;
  uint16_t    CxlQosSchedGap;
  bool        CxlQosVariableGap;
  bool        CxlQosRdspIncMode;
  uint8_t     CxlQosTimerDecNum;
  uint8_t     CxlQosRdspIncNum;
  uint8_t     CxlQosWrrspIncNum;
  bool        CfgSevSnpSupport;
