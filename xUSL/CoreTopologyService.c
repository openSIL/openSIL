  ...
  )
{
  ...
  Status = DfIp2IpApi->DfGetSystemInfo(SilContext, &NumberOfProcessors, NULL, NULL, NULL, NULL);
  if (Status != SilPass) {
  ...
  )
{
  ...
  Status = DfIp2IpApi->DfGetProcessorInfo(SilContext, Socket, &NumberOfDies, NULL);
  if (Status != SilPass) {
 ...
 *                               can be passed to openSIL. The host firmware is responsible
 *                               for initializing the SIL_CONTEXT structure.
 * @param   ApobCcdLogToPhysMap  Pointer to the APOB CCD logical to physical map structure
 * @param   Socket               Zero-based socket number to check.
 * @param   Die                  The target die's identifier within Socket.
 * @param   NumberOfCcds         Pointer to the number of enabled CCDs on the given socket / die.
 *
 ...
 * @param   Socket               Zero-based socket number to check.
 * @param   Die                  The target die's identifier within Socket.
 * @param   Ccd                  The target CCD's identifier within die
 ...
 * @param   Die                  The target die's identifier within Socket.
 * @param   Ccd                  The target CCD's identifier within die
 * @param   Complex              The target Complex's identifier within ccd
 * @param   NumberOfCcds         Pointer to the number of enabled CCDs on the given socket / die.
 *
 * @retval SilPass               The core topology information was successfully retrieved.
 ...
 * @param   Ccd                  The target CCD's identifier within die
 * @param   Complex              The target Complex's identifier within ccd
 * @param   Core                 The target Core's identifier within core
 * @param   NumberOfThreads      Pointer to the number of enabled threads per core on the given socket / die.
 *
 * @retval SilPass               The core topology information was successfully retrieved.
 ...
 *                               for initializing the SIL_CONTEXT structure.
 * @param   ApobCcdLogToPhysMap  Pointer to the APOB CCD logical to physical map structure
 * @param   Socket               On input, caller provides logical socket number,
 *                               On output, it is physical socket number.
 * @param   Die                  On input, caller provides logical die number,
 *                               On output, it is physical die number.
 * @param   Ccd                  On input, caller provides logical CCD number,
 *                               On output, it is physical CCD number.
 * @param   Complex              On input, caller provides logical complex number,
 *                               On output, it is physical complex number.
 * @param   Core                 On input, caller provides logical core number,
 ...
 * @param   Socket               Zero-based socket number to check.
 * @param   Die                  The target die's identifier within Socket.
 * @param   MaxNumberOfCcds      Pointer to the maximum number of enabled CCDs on the given socket / die.
 * @param   MaxNumberOfComplexes Pointer to the maximum number of enabled complexes on the given socket / die.
 * @param   MaxNumberOfCores     Pointer to the maximum number of enabled cores per complex on the given
 *                               socket / die.
 * @param   MaxNumberOfThreads   Pointer to the maximum number of enabled threads per core on the given socket / die.
 *
 * @retval SilPass                The core topology information was successfully retrieved.
  ...
  )
{
  ...
  Status = ApobIp2IpApi->ApobGetCcdLogToPhysMap(SilContext, Socket, Die, &ApobCcdLogToPhysMap);
  ...
  if (Status == SilPass) {
    ...
    *MaxNumberOfThreads = 1;
    Status = GetCcdCountOnDie(SilContext, &ApobCcdLogToPhysMap, Socket, Die, &CcdCount);
    if (Status != SilPass) {
      CcdCount = 0;
    }
    ...
    for (CcdLoop = 0; CcdLoop < CcdCount; CcdLoop++) {
      ...
      for (ComplexLoop = 0; ComplexLoop < ComplexCount; ComplexLoop++) {
        ...
        //
        // Thread count is either 1 or 2. Check if the count is 2.
        //
  ...
  )
{
  ...
  if (*Socket == CORE_TOPOLOGY_V3_ITERATION_START) {
    *Socket = 0;
    ...
    NumOfSocket = GetSocketCountOnSystemMax(SilContext);
 ...
 *                              for initializing the SIL_CONTEXT structure.
 * @param   Socket              Zero-based Socket number.
 * @param   Die                 Zero-based Die number.
 *
 * @return CORE_TOPOLOGY_ITERATION_RESULT
 *
 **/
  ...
  )
{
  ...
  if ((*Socket == CORE_TOPOLOGY_V3_ITERATION_START) ||
    ...
    *Die = 0;
    ...
    NumOfDie = GetDieCountOnSocketMax(SilContext, *Socket);

    if ((*Die + 1) == NumOfDie) {
      // Loop next socket
      IterationResult = CoreTopologyV3IterateSocket(SilContext, Socket);
 ...
 * @param   Socket                Zero-based Socket number.
 * @param   Die                   Zero-based Die number.
 * @param   Ccd                   Zero-based Ccd number.
 *
 * @return CORE_TOPOLOGY_ITERATION_RESULT
 *
 **/
  ...
  )
{
  ...
  if ((*Socket == CORE_TOPOLOGY_V3_ITERATION_START) ||
    ...
    *Ccd = 0;
    ...
    Status = GetCcdCountOnDie(SilContext, ApobCcdLogToPhysMap, *Socket, *Die, &NumOfCcd);
    if (Status == SilPass) {
      if ((*Ccd + 1) == NumOfCcd) {
        // Loop next die
        IterationResult = CoreTopologyV3IterateDie(SilContext, Socket, Die);
 ...
 * @param   Die                   Zero-based Die number.
 * @param   Ccd                   Zero-based Ccd number.
 * @param   Complex               Zero-based Complex number.
 *
 * @return CORE_TOPOLOGY_ITERATION_RESULT
 *
 **/
  ...
  )
{
  ...
  if ((*Socket == CORE_TOPOLOGY_V3_ITERATION_START) ||
    ...
    *Complex = 0;
    ...
    Status = GetComplexCountOnCcd(SilContext, ApobCcdLogToPhysMap, *Socket, *Die, *Ccd, &NumOfComplex);
    if (Status == SilPass) {
      if ((*Complex + 1) == NumOfComplex) {
        // Loop next Ccd
        IterationResult = CoreTopologyV3IterateCcd(SilContext, ApobCcdLogToPhysMap, Socket, Die, Ccd);
 ...
 * @param   Ccd                   Zero-based Ccd number.
 * @param   Complex               Zero-based Complex number.
 * @param   Core                  Zero-based Core number.
 *
 * @return CORE_TOPOLOGY_ITERATION_RESULT
 *
 **/
  ...
  )
{
  ...
  if ((*Socket == CORE_TOPOLOGY_V3_ITERATION_START) ||
    ...
    *Core = 0;
    ...
    Status = GetCoreCountOnComplex(SilContext, ApobCcdLogToPhysMap, *Socket, *Die, *Ccd, *Complex, &NumOfCore);
    if (Status == SilPass) {
      if ((*Core + 1) == NumOfCore) {
        // Loop next Complex
        IterationResult = CoreTopologyV3IterateComplex(SilContext, ApobCcdLogToPhysMap, Socket, Die, Ccd, Complex);
 ...
 * @param   Complex             Zero-based Complex number.
 * @param   Core                Zero-based Core number.
 * @param   Thread              Zero-based Thread number.
 *
 * @return CORE_TOPOLOGY_ITERATION_RESULT
 *
 **/
  ...
  )
{
  ...
  if ((*Socket == CORE_TOPOLOGY_V3_ITERATION_START) ||
    ...
    *Thread = 0;
    ...
    Status = GetThreadCountOnCore(SilContext, ApobCcdLogToPhysMap, *Socket, *Die, *Ccd, *Complex, *Core, &NumOfThread);
    if (Status == SilPass) {
      if ((*Thread + 1) == NumOfThread) {
        // Loop next Core
        IterationResult = CoreTopologyV3IterateCore(SilContext, ApobCcdLogToPhysMap, Socket, Die, Ccd, Complex, Core);
