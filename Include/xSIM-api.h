 ...
 *   coming off of reset. It places the silicon into a state where more
 ...
 *   further details.
 *
 *   SocList - The platform may support one or more SoC's in it socket(s). This
...
/** @brief Header structure for SIL Input & Output Blocks
 *
...
/**
 * @brief SIL Host API TimePoints
 * @details The AMD silicon needs to be initialized in a specific
 * sequence. Intermingled in this sequence are actions required to be
 * performed by the Host. This means transfer of control must occur at
 * well defined points in time. These time points are:
 *  - TP1 - After CPU reset and before PCIe resource allocation
 *    ('enumeration')
 *  - TP2 - After PCIe resource allocation and before ??
 *  - TP3 - Late  in POST, just before OS/payload loading
 *  The Host is required to call each time point at the specified time.
 *  This is needed so that the silicon completes its full
 *  initialization. For more information, please see the openSIL
 *  Architecture Design specification.
 */
typedef enum {
  SIL_TP1,                    ///< In TP1 SIL assign the memory to the Each IPs
 ...
 *
 * 1. Initializing the global pointer to the Host allocated SIL
 *    memory block.
