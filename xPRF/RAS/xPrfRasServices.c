  ...
  )
{
  ...
  for (SocketId = 0; SocketId < NumberOfInstalledProcessors; SocketId++) {
    ...
    for (DieId = 0; DieId < TotalNumberOfDie; DieId++) {
      ...
      SegNumber = (uint8_t)(BusNumberBase / MAX_PCI_BUS_NUMBER_PER_SEGMENT);
      //Get UMC Harvest FUSE
      BusNumberBase = (uint32_t)((((uint32_t)SegNumber & 0xFFFF) << 8) | (BusNumberBase & 0xFF));
      ...
      for (Index = 0; Index < DimmFruTextTable->TableEntryNum;
        ...
        if ((DimmFruTextTable->FrutextEntry[Index].Socket == (uint16_t)SocketId)
          ...
          if (0 == (SmuFuse &
            ...
            Status = RasApi->UpdateFruTextToUmc(BusNumberBase,
  ...
  )
{
  ...
  if (RasCpuInfo->ProcessorNumber == 0) {
    XPRF_TRACEPOINT(SIL_TRACE_ENTRY, "\n");
  }
  ...
  if (Status == SilPass) {
    RasApi->UpdateMcaFruText(RasCpuInfo);
  }
