@page Kconfig-top Kconfig Configuration Tool
@tableofcontents
...
The script file should be executed while path'd to the openSIL root folder.
...
### Expectations:
- Run this tool from the openSIL root directory
    * ``` > util\Kcfg menu MyPlat ```
 ...
 * Command -  action to perform. Use one of the following:
   ...
   - 'update'  - After creating a [PlatName] file with 'menu',
                   ...
                   the [platName].h file)
   - 'allyes'  - For use with CI compile tests, this creates
                   a file with all options='yes'
