...
KcfgHelp () {
    ...
    echo "*        'allyes'  - For use with CI compile tests, this creates  *"
    echo "*                    a file with all options='yes'                *"
    echo "*                                                                 *"
    ...
    echo "*               e.g.  'Purico_SilCfg' or 'Purico'  may be used.   *"
    echo "*                                                                 *"
...
PlatName () {
    ...
    if test -z ${2} ; then
        echo "*** ERROR: PlatName parameter missing"
        KcfgHelp
        exit 1
    fi
    if test -f "configs/${2}" ; then
        ...
        # file was not found, check for allyes variation
        if [[ ${1} == allyes ]] ; then
            export KCONFIG_CONFIG=$2-AllYes
            export KCONFIG_AUTOHEADER=${KCONFIG_CONFIG}.h
            if test -f "configs/${2}-AllYes" ; then
                # cmd is AllYes and param was base name, add '-AllYes', proceed...
                cp -p --update --verbose  configs/${KCONFIG_CONFIG}* .
            else
                # No AllYes file, make one with defaults
                python ${PYTHONPATH}/alldefconfig.py
            fi
        else
            # file was not found, create default IFF cmd=menu
            if [[ ${1} == menu ]] ; then
                # Only for the 'menu' command should we create the defaults, else error
    ...
    echo "** Verified Plat filenames as: ${KCONFIG_CONFIG}, ${KCONFIG_AUTOHEADER}"
};
...
case $1 in
  ...
  ( allyes )
    ...
    retcode=$?
    if [ ${retcode} -eq 0 ] ; then
        echo "** Generating the 'All Yes' config for CI testing"
        echo "**   to ${KCONFIG_CONFIG}, ${KCONFIG_AUTOHEADER}"
        python ${PYTHONPATH}/allyesconfig.py
        python ${PYTHONPATH}/genconfig.py
    fi
    ;;
